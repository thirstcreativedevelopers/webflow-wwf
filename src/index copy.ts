...
collectionItems.forEach(function (item) {
  ...
  if (window.innerWidth >= 1024) {
    ...
    marker.addEventListener('click', function () {
      // Remove the class "actives" from all markers
...
// Close overlay and popup triggred

const popupBtn = document.querySelector('.ig-popup .btn');
const popup = document.querySelector('.ig-popup');
const overlay = document.querySelector('.ig-overlay');

// Function to add the 'closed' class to both elements
...
popupBtn.addEventListener('click', addClosedClass);
overlay.addEventListener('click', addClosedClass);
